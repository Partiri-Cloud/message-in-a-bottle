...
func (r *NotificationRepository) MarkSeen(ctx context.Context, envID, subscriberID, id bson.ObjectID) error {
	now := time.Now()
...
func (r *NotificationRepository) MarkRead(ctx context.Context, envID, subscriberID, id bson.ObjectID) error {
	now := time.Now()
	_, err := r.col.UpdateOne(ctx,
...
func (r *NotificationRepository) MarkArchived(ctx context.Context, envID, subscriberID, id bson.ObjectID) error {
	now := time.Now()
	_, err := r.col.UpdateOne(ctx,
...
func (r *NotificationRepository) BulkMarkRead(ctx context.Context, envID, subscriberID bson.ObjectID, ids []bson.ObjectID) error {
	now := time.Now()
...
func (r *NotificationRepository) BulkMarkSeen(ctx context.Context, envID, subscriberID bson.ObjectID, ids []bson.ObjectID) error {
	now := time.Now()
	_, err := r.col.UpdateMany(ctx,
		bson.M{"_id": bson.M{"$in": ids}, "environmentId": envID, "subscriberId": subscriberID},
		bson.M{"$set": bson.M{"seen": true, "seenAt": now, "updatedAt": now}},
